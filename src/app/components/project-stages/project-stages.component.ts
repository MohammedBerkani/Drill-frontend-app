...
export class ProjectStagesComponent implements OnInit {
  ...
  //   this.apiService.getOperators().subscribe((data) => {
  //     this.Operator = data;
  //   });
  // }
   getOneProject() {
  ...
  Assign(operator: any) {
    if(window.confirm(`Are sure you wanna assign  ${operator.name} on this project` )){
      console.log('qsd');
      ...
      this.http
        .patch(url, { operating: true }, { headers: this.headers })
        .subscribe();
  ...
  Dismiss(operator: any) {
    if(window.confirm(`Are sure you wanna dismiss ${operator.name} from this project`)){
      let id_S = operator._id;
      console.log(id_S);
      let id_P = this.actRoute.snapshot.paramMap.get('id2');
      let id_sup = this.actRoute.snapshot.paramMap.get('id');
      ...
      this.http
        .patch(url, { operating: false }, { headers: this.headers })
        .subscribe();
      return window.location.reload();
       }
